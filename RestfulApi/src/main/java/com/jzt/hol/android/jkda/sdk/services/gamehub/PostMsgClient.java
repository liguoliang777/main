...
public class PostMsgClient extends PostMsgBaseClient<NormalDataBean> {
    PostMsgBodyBean bean;

    public PostMsgClient(Context cxt, PostMsgBodyBean bean) {
        super(cxt);
        this.bean = bean;
    }
    ...
    protected Observable<NormalDataBean> requestService(GameService askDoctorService) {
        return askDoctorService.addPost(HostDebug.AppJson, bean);
    }
}
