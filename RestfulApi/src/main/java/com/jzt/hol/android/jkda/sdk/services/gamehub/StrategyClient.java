...
public class StrategyClient extends PostMsgBaseClient<GameHubMainBean> {
    GameHubMainBodyBean bean;

    public StrategyClient(Context cxt, GameHubMainBodyBean bean) {
        super(cxt);
        this.bean = bean;
    }
    ...
    protected Observable<GameHubMainBean> requestService(GameService askDoctorService) {
        return askDoctorService.raiderPostList(HostDebug.AppJson, bean);
    }
}
